  ...
  <main class="drum-machine">
    <form action>
      ...
      <div class="bpm-and-play" aria-labelledby="dm-main-heading">
        ...
        <div class="play-stop">
          <button
            :aria-pressed="meta.isPlaying.toString()"
      ...
      <div class="tracks" aria-labelledby="dm-tracks-heading">
        ...
        <fieldset
          role="group"
          :aria-labelledby="(sound.name + '-track-legend') | slugify"
          v-for="sound in sounds"
          class="track"
        >
          <legend :id="(sound.name + '-track-legend') | slugify">
            {{ sound.name }} track
          </legend>
          <div class="track-main">
            <div class="track-addRemove">
              <button
                ...
                @click.prevent="removeBeat(sound)"
              >
            ...
            <div
              class="track-beats"
              role="group"
              :aria-labelledby="(sound.name + '-track-label') | slugify"
            >
              <span
                class="track-label"
                :id="(sound.name + '-track-label') | slugify"
              >
                <span class="vh">{{ sound.length }} quarter beats for</span>
                {{ sound.name }}
              </span>
              <div
                v-for="n in sound.length"
                :style="{ width: meta.beatsLength }"
              >
                <input
                  type="checkbox"
                  :id="(sound.name + '-beat-' + n) | slugify"
                  :value="n"
                  v-model="sound.active"
                />
                <label
                  :for="(sound.name + '-beat-' + n) | slugify"
                  :id="(sound.name + '-beat-' + n + '-label') | slugify"
                  ><span class="vh">Quarter Beat {{ n }}</span></label
                >
              </div>
            </div>
            <div class="track-muteSettings">
              <button
                @click.prevent="sound.muted = !sound.muted"
                :aria-pressed="sound.muted.toString()"
                aria-label="mute"
              >
                <mute-icon></mute-icon>
              </button>
              <button
                class="open-settings"
                @click.prevent="sound.expanded = !sound.expanded"
                :aria-expanded="sound.expanded.toString()"
                aria-label="settings"
              >
          ...
          <transition name="slide">
            <fieldset
              role="group"
              aria-labelledby="track-settings-label"
              v-if="sound.expanded"
            ...
            >
              ...
              <div class="setting">
                ...
                <div>
                  <label for="prob-chance" class="vh"
                    >Chance
                    <span class="vh">(probability numerator)</span></label
                  >
                  <input
                    type="number"
                    id="prob-chance"
                    min="1"
                    v-model="sound.probability.chance"
                  />
                  <span aria-hidden="true" class="between">in</span>
                  <label for="prob-in" class="vh"
                    >In <span class="vh">(probability denominator)</span></label
                  >
                  <input
                    type="number"
                    id="prob-in"
                    min="1"
                    v-model="sound.probability.in"
                  />
                </div>
              </div>
              <div class="setting" v-if="meta.detuneSupport">
                ...
                <div>
                  <input
                    ...
                    min="0"
                    max="100"
                    v-model="sound.fluctuationLevel"
                    aria-describedby="fluc-desc"
                  />
              ...
              <div class="setting">
                ...
                <div class="checkbox-group">
                  <div
                    v-for="otherSound in sounds"
                    v-if="otherSound.name !== sound.name"
                  >
                    <input
                      type="checkbox"
                      :id="
                        (sound.name + '-overrides-' + otherSound.name) | slugify
                      "
                      :value="otherSound.name"
                      v-model="sound.overrides"
                    />
                    <label
                      :for="
                        (sound.name + '-overrides-' + otherSound.name) | slugify
        ...
        <aside class="stats">
          ...
          <p>
            Polymetric pattern length: {{ meta.compoundLength / 4 }} beats ({{
              meta.compoundLength
            }}
            &#x00bc;-beats)
            <button
...
var defaultState = {
  sounds: [
    ...
    {
      name: "bassoon half A#4",
      url: "sounds/sample_library/bassoon_sebastian/bassoon_half_1_out_A%234.wav",
      buffer: null,
      length: 8,
      current: 1,
      active: [],
    ...
    {
      name: "bassoon half C5",
      url: "sounds/sample_library/bassoon_sebastian/bassoon_half_2_out_C5.wav",
      buffer: null,
      length: 8,
      current: 1,
      active: [],
    ...
    {
      name: "bassoon half D5",
      url: "sounds/sample_library/bassoon_sebastian/bassoon_half_3_out_D5.wav",
      buffer: null,
      length: 8,
      current: 1,
      active: [],
    ...
    {
      name: "bassoon quarter B4",
      url: "sounds/sample_library/bassoon_sebastian/bassoon_quarter_1_out_B4.wav",
      buffer: null,
      length: 8,
      current: 1,
      active: [],
    ...
    {
      name: "bassoon quarter C5",
      url: "sounds/sample_library/bassoon_sebastian/bassoon_quarter_2_out_C5.wav",
      buffer: null,
      length: 8,
      current: 1,
      active: [],
    ...
    {
      name: "bassoon quarter D5",
      url: "sounds/sample_library/bassoon_sebastian/bassoon_quarter_3_out_D5.wav",
      buffer: null,
      length: 8,
      current: 1,
      active: [],
    ...
    {
      name: "bassoon staccatto A#4",
      url: "sounds/sample_library/bassoon_sebastian/bassoon_staccatto_1_out_A%234.wav",
      buffer: null,
      length: 8,
      current: 1,
      active: [],
    ...
    {
      name: "bassoon staccatto C5",
      url: "sounds/sample_library/bassoon_sebastian/bassoon_staccatto_2_out_C5.wav",
      buffer: null,
      length: 8,
      current: 1,
      active: [],
    ...
    {
      name: "bassoon staccatto C#5",
      url: "sounds/sample_library/bassoon_sebastian/bassoon_staccatto_3_out_C%235.wav",
      buffer: null,
      length: 8,
      current: 1,
      active: [],
...
console.log("DEBUG", defaultState);
...
export default {
  ...
  methods: {
    ...
    soundLoader(path) {
      ...
      soundObject.play = (volumeVal, time, fluctuationLevel, animElem) => {
        ...
        if (this.meta.detuneSupport) {
          ...
          var bend =
            Math.floor(Math.random() * fluctuationLevel) +
            -Math.abs(fluctuationLevel);
    ...
    playOrNot(sound, currentIsActive) {
      ...
      var soundID =
        this.slugify(sound.name + "-beat-" + sound.current) + "-label";
    ...
    link() {
      ...
      this.sounds.forEach(function (sound) {
        ...
        var part =
          ...
          "=" +
          encodeURIComponent(JSON.stringify(persisted));
        parts.push(part);
      });
  ...
  watch: {
    sounds: {
      handler() {
        ...
        var uniqueLengths = lengthInfo.lengths.filter(
          (v, i, a) => a.indexOf(v) === i
        );
